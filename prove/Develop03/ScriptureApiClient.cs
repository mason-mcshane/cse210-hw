...
public class ScriptureApiClient
{
    ...
    public ScriptureApiClient()
    {
        ...
        _client.BaseAddress = new Uri("https://openscriptureapi.org/api/scriptures/v1/lds/en/volume/");
    }
    public async Task<string> GetScriptureTextAsync(Reference reference)
    {
        try
        {
            ...
            {
                throw new Exception($"Failed to retrieve scripture passage. Status code: {response.StatusCode}");
            }
        }
        catch (Exception ex)
        {
